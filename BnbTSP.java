...
import java.util.*;
//This implementation employs a Branch and Bound algorithm
...
public class BnbTSP
{
    ...
    private int[] final_path = new int[N + 1];
    ...
    //Goes thru the row to see first the min edge
    ...
    // //Goes thru the row to see second the min edge
     ...
     */
    public void TSPRec(int[][] adj, int curr_bound, int curr_weight, int level, int[] curr_path)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (adj[curr_path[level-1]][i] != 0 && !visited[i])
            {
                ...
                if (curr_bound + curr_weight < final_res)
                {
                    ...
                    // call TSPRec for the next level recursively
    ...
    public void TSP(int[][] adj)
    {
        ...
        // Calculate initial lower bound for the root node using
        // the formula 1/2 * (sum of first min + second min) for all edges.
